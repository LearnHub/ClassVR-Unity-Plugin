...
- `AndroidIntent.Extras` exposes an intent's extras as a string key/value lookup

### Changed

- Upgraded CVR Unity Java Plugin to version 9 to add support for Intent Extra serialization
- `AndroidIntent.Extras` is now `IReadOnlyDictionary<string, string>` rather than `string`. It was declared but never populated, so not marked as breaking
